bf_report_manager = function() {
	...
	this.init = function() {
		...
		var catcha_buttons = $$('input.flag_comment_submit');
		catcha_buttons.each( function(i) {
			i.observe('click', function(e) {
				e.stop();
				report_manager.flag_comment(e);
				return false;
	...
	this.flag_comment = function(e) {
		...
		var sr = function(resp) {
			bf_catcha.catcha_answer = '';
			$('flag_comment').hide();
			$$('span[comment_id="'+id+'"]')[0].addClassName('reporting');
		}.bind(this);
		var er = function() { this.error(el); }.bind(this);

		var user = new BF_User;
		var user_data = user.getUserInfo();

		if( (el.id && el.id=="flag_submit") || confirm('Report this comment?')  ) {
			var params = { 
				comment_id: id, 
				action: 'flag_contribution',
				anonymus: user.isLoggedIn() ? 0 : 1, 
				user_id: user_data.userid
			};
			
			(new BF_Request()).request( '/buzzfeed/flag_comment', {
				method: 'post',
				parameters: params,
				onSuccess: sr,
				onFailure: er,
				onCatcha: function(resp) {
					flagComment.move_it(id);
					if ( bf_catcha.catcha_answer ) {
						alert('Wrong answer!');
					}
				}
			});			
